...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        ...
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"最多可以选择 %d 张图片.", nil), self.maximumImagesCount];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"您一次最多选择 %d 张图片.", nil), self.maximumImagesCount];
